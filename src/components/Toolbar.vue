  ...
  <header class="toolbar">
    ...
    <nav class="breadcrumbs" aria-label="路径">
      <template v-for="(crumb, i) in breadcrumbs" :key="crumb.path">
        ...
        <button
          :class="['crumb', i === breadcrumbs.length - 1 && 'active']"
    ...
    <div class="toolbar-actions">
      ...
      <button
        :class="['icon-button', sharePortLocked && 'active']"
        @click="togglePortLock"
        v-html="sharePortLocked ? icons.lock : icons.unlock"
      />
      ...
      <div class="segmented">
        <button
          :class="['icon-button', view === 'list' && 'active']"
          @click="setView('list')"
          v-html="icons.list"
        />
        <button
          :class="['icon-button', view === 'grid' && 'active']"
          @click="setView('grid')"
          v-html="icons.grid"
        />
      </div>
      ...
      <button
        :class="[
          'icon-button',
          ocserverRunning && 'ocserver-active',
          ocserverLoading && 'loading',
        ]"
...
async function chooseRoot() {
  ...
  if (!selected) return;
  store.root = selected;
  localStorage.setItem("oFinder-root", selected);
  store.backStack = [];
  store.forwardStack = [];
  emit("navigate", "", false);
}
...
function setView(v) {
  store.view = v;
  localStorage.setItem("oFinder-view", v);
}
